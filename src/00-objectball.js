...
function biggestShoeRebounds(){
  let biggestShoe = 0;
  let playerBigShoe;
  let playerBigShoeRebounds;
  let game = gameObject()
  for (let gameKey in game){
    gameKey = game[gameKey].players
    for(let i in gameKey){
      if(gameKey[i].shoe > biggestShoe){
        biggestShoe = gameKey[i].shoe
        playerBigShoe = gameKey[i]
        playerBigShoeRebounds = gameKey[i].rebounds
      }
    }
  }  
  return(playerBigShoeRebounds)
}
